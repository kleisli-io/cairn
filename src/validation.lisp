;;; Task name validation: a cheap quality gate that keeps meaningless
;;; identifiers ("P1", "phase-1", "stuff") out of the task graph. Pure; depends
;;; only on the standard library.

(defpackage #:kli/cairn/validation
  (:use #:cl)
  (:export
   #:validate-task-name
   #:validation-result
...
(in-package #:kli/cairn/validation)
...
(define-condition name-improvement-failed (error)
  ...
  (:documentation
   "Signalled when a task name fails validation and no valid slug can be
synthesised from the accompanying description.")
...
(defun split-kebab (name)
  "Split kebab-case NAME into a list of parts."
  (let ((parts nil)
...
(defun numeric-p (s)
  "True when S is purely numeric."
  (and (> (length s) 0) (every #'digit-char-p s)))

(defun letter-number-p (s)
  "True when S is a single letter followed by numbers (P1, T2)."
  (and (>= (length s) 2)
...
(defun meaningful-part-p (p)
  "True when P is meaningful (not just a number or letter+number)."
  (and (> (length p) 1)
       (not (numeric-p p))
       (not (letter-number-p p))))

(defun filter-structural-parts (parts)
  "Remove a leading prefix and numbers, keeping the semantic core."
  (let ((result parts))
...
(defun has-good-grammar-p (parts)
  "True for valid naming grammar patterns."
...
(defun validate-task-name (name)
  "Validate NAME is semantically descriptive. Returns a VALIDATION-RESULT."
...
(defun slugify (text &key (max-words 4))
  "Convert TEXT to a kebab-case slug, filtering stopwords and limiting to
MAX-WORDS meaningful words."
  (let* ((normalized (substitute #\Space #\/ (substitute #\Space #\_ text)))
         ...
         (words (remove-if (lambda (w) (= (length w) 0))
                           (loop for start = 0 then (1+ end)
                                 ...
                                 collect (string-downcase
                                          (subseq cleaned start (or end (length cleaned))))
                                 while end)))
